...
lasso <- function(n){
  ...
  for(i in 1:nrep){
    ...
    ###Run a cross validation of Lasso to obtain a lambda with minimal deviance
    ...
    d=data.matrix(fit$df)
    l=data.matrix(fit$lambda)
    dflam=cbind(d,l)
    df=dflam[(dflam[,2] %in% cv.fit$lambda.min),1]
    df=fit$df
    stat[i,1]=df
